...
/*PROTECTED REGION ID(usingRentacar2Gen.ApplicationCore.CEN.RentaCar2_Coche_desreservarCoche) ENABLED START*/
...
{
public void DesreservarCoche (int p_id)
{
        ...
        if (cocheEN.Disponible == Enumerated.RentaCar2.EstadoEnum.reservado) {
                cocheEN.Disponible = Enumerated.RentaCar2.EstadoEnum.disponible;

                _ICocheRepository.ModifyDefault (cocheEN);
        }
        else{
                throw new Exception ("El coche ya estaba disponible.");
        }
